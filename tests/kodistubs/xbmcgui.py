...
class WindowXMLDialog(object):
    """Enough of Kodi's custom-window base class for the module to import.

    Deliberately not a working window: doModal needs a running Kodi and the
    skin XML only renders inside one, so nothing off the device can exercise
    the window itself. What the tests drive is the Board behind it, which is
    why that is a plain object with no Kodi in it.
    """
    ...
    def doModal(self):
        raise AssertionError(
            'doModal needs a running Kodi; test the Board instead')
