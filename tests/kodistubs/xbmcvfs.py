...
# When True, exists() answers False for a directory path written with a
# trailing slash. Real Kodi builds differ on this and some refuse it over SMB,
# which is how a folder that is plainly there gets reported missing.
DIRS_REFUSE_TRAILING_SLASH = False
...
def exists(path):
    ...
    if not real:
        return False
    if (DIRS_REFUSE_TRAILING_SLASH and path.endswith('/')
